...
%<USER_OPTIONS> tag used by WRITEOPTIONSFILE, do not remove!
...
% PROVISIONAL: important user input
    ...
    Defaults.albedo = 0.2;
    Defaults.soiling = 0.0;
    
%     Defaults.sensors = struct('normal',{},'type',{},'id',{}); 
%      
%     % Defaults.sensors(1).normal = [0;0;1]; % GHI only
%     % Defaults.sensors(1).type = 'GHI_B';   
       
    Defaults.sensors = table();
    
    % Defaults.sensors{1,'id'} = {'GHI'};
    % Defaults.sensors{1,'type'} = {'GHI_B'};  % Class B Pyranometer (see SENSORUNCERTAINTY)
    % Defaults.sensors{1,'el'} = 90;
    % Defaults.sensors{1,'az'} = 0;
    ...
    Defaults.meteo.separationmodel = ''; % best available
    ...
    Defaults.meteo.fillgaps = 0;    % Interpolate and keep(*) missing irradiance values
                                    % up to X samples ahead/before valid data. NOT RECOMMENDED!
                                    % NOTE: missing ancilliary variables (e.g. Ta, Patm,..) are
                                    % interpolated in any case.
                                   
    Defaults.meteo.fitclearsky = 0;      % attempt to correct bias in clear-sky model
                                         % 'auto' = true if McClear data is not available.
    Defaults.meteo.clearskycheck = 0;    
                                         
    Defaults.meteo.minCSfraction=0.05;   % Minimum required fraction of clear-sky-samples
    
    Defaults.minGHI = 2.0;               % Min. GHI/POA (W/m²) for daylight/PV-production
    Defaults.minSunEl = 0.0;             % Min. solar elevation (degrees >= 0) for daylight.
    ...
    % Defaults.outliers.withintol = 'flag'; % 'fix','nan'
    % Defaults.outliers.beyondtol = 'nan';  % 'flag','nan'
    
    Defaults.meteoQC.basic = false;
    Defaults.meteoQC.independent = false;
    Defaults.meteoQC.sequence = 'all';
    ...
    Defaults.IAM = 'refraction';  % {'none','ashrae','refraction','martin-ruiz','hcpv'}
                                    % See default parameters for each model in CHECKIAM, or
                                    % use a structure, e.g. struct('model','ashrae','b',0.05)
...
% (Normally) Hidden simulation options
    ...
    % Parallel execution
    Defaults.runparallel = false;       % split computationally-intensive tasks over several MATLAB instances  
    ...
    Defaults.RelTol = 1e-3;             % Relative tolerance (simulation accuracy)
