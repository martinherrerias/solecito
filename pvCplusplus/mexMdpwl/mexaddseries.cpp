...
void mexaddseries(double ***array, mwSize* inLength, mwSize size, double* tol, double* lim, mwSize tolSize, mwSize limSize, double* outarray1, double* outarray2, unsigned int& length)
{
#ifdef TIMER
...
#endif
    // addseries
    output = MonoDescPWL(set, Dim::hori, vecTol, vecLim);
#ifdef TIMER
    std::cout<<"Pure C++ addseries overhead...";
                 ...
                 int nrhs, const mxArray *prhs[])
{
   ...
   double sWholeTime, eWholeTime;
    ...
    mwSize i, j, nrows0, ncols0, nrows1, ncols1, size, sizeTol, sizeLim, maxElem;
#ifdef TIMER
...
//    lim = mxGetPr(prhs[size+2]);
    ...
    TIMESTAMP( sCallTime );
#endif
    mexaddseries(inMatrix, inLength,  size, tol, lim, sizeTol, sizeLim, outMatrix1, outMatrix2, *(unsigned int*)mxGetData(plhs[2]));
#ifdef TIMER
    TIMESTAMP( eCallTime );
